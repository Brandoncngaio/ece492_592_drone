...
bbox = (225, 175, 200, 200) #changed box size
...
try:
    ...
    while run:
        ...
        p1 = (int(bbox[0]), int(bbox[1]))
        p2 = (int(bbox[0] + bbox[2]), int(bbox[1] + bbox[3]))
        cv2.rectangle(frame, p1, p2, (128,128,0), 2, 1)
        cv2.imshow("Tracking", frame)
        ...
        if cv2.waitKey(1) & 0xFF == ord('c'): # press 'c' to send attitude adjustment
            controlVehicle = not controlVehicle
            print("controlVehicle has been toggled")
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'): # press 'c' to send attitude adjustment
